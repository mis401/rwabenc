...
import { cancelSession, cancelSessionFailure, createSession, createSessionSuccess, joinSession, joinSessionSuccess, loadMessages, loadMessagesFailure, loadMessagesSuccess, loadUserSessions, loadUserSessionsFailure, loadUserSessionsSuccess, navigateToSession, sessionSelected, sessionSelectedFailure, sessionSelectedSuccess } from "./session.actions";
...
export class SessionEffects {
    ...
    joinSession$ = createEffect(() => this.actions$.pipe(
        ofType(joinSession),
        switchMap((action) => this.sessionService.joinSession(action.session, action.user).pipe(
            map((session) => joinSessionSuccess({session})),
            catchError((error) => of(cancelSessionFailure({error})))
        )),
    ))
}
